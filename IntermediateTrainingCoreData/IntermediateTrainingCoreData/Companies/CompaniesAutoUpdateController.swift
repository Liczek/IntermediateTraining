...
//
//  Created by Paweł Liczmański on 26/09/2018.
...
class CompaniesAutoUpdateController: UITableViewController, NSFetchedResultsControllerDelegate {
	...
	override func viewDidLoad() {
		...
		navigationItem.leftBarButtonItems = [
			...
			UIBarButtonItem(title: "Remove", style: .plain, target: self, action: #selector(handleRemove))
		]
	...
	@objc private func handleAdd() {
		...
		do {
			...
			print("Failed to add new company", saveErr)
		}
	}
	...
	@objc private func handleRemove() {
		...
		do {
			let companiesWithA = try context.fetch(request)
			for company in companiesWithA {
				context.delete(company)
				
				do {
					try context.save()
				} catch let err {
					print("Failed to save context without company with A letter", err)
				}
			}
		} catch let err {
			print("Failed to fetch Companies with A letter", err)
		}
	}
